...
export function doesSlugUserExist(slug) {
...
export function createSlugUser(name, lastname) {
  const fullname = name.toLowerCase() + lastname.toLowerCase();
  
  
  let result = fullname;
  
  let exist = doesSlugUserExist(fullname);
  
  if (exist.length) {
    let n = 1;
    let a = doesSlugUserExist(result);
    let c = a.length;
